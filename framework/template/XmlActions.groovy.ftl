...
<#--spreading code：entity-sequenced-id-primary-dynamic 标签实现-->
<#macro "entity-sequenced-id-primary-dynamic">
${.node["@value-field"]}.setDynamicSequencedIdPrimary()
</#macro>
...
<#macro "check-errors">    if (ec.message.errors) return
    ...
    <#--spreading code：try catch finally标签实现-->
<#macro "catch"><#-- ignore me--></#macro>
<#macro "finally"><#-- ignore me--></#macro>
<#--<#macro "try">
    try{<#recurse .node/>}catch (Throwable t) {ec.logger.log(t.toString ())
    ec.message.addError(t.toString ())}finally{<#if .node["catch"]?has_content>
    if (ec.message.hasError()) {<#recurse .node["catch"][0]/>}</#if>
    <#if .node["finally"]?has_content><#recurse .node["finally"][0]/></#if>
    ec.message.clearErrors()}
</#macro>-->
<#macro "try">
    try{
        ...
        <#if .node["catch"]?has_content><#recurse .node["catch"][0]/></#if>
        ec.logger.info(e.getMessage())
        }finally{
    <#if .node["finally"]?has_content><#recurse .node["finally"][0]/></#if>
        }
</#macro>

<#--spreading code：transaction标签实现-->
<#macro "transaction">
    suspendedTransaction = false;
    try {
    if (ec.transaction.isTransactionInPlace()) suspendedTransaction = ec.transaction.suspend();
    boolean beganTransaction = ec.transaction.begin(${.node.@timeout!"null"});
    try {
    <#recurse .node/>
    } catch (Throwable t) {
    ec.logger.log("error","a throwable occurs in tag(transaction):["+t.toString ()+"]",t);
    ec.message.addError(t.toString ());
    ec.transaction.rollback(beganTransaction, "the nested transaction rolled back because occurred an exception ", t);
    } finally {
    if (ec.transaction.isTransactionInPlace()) ec.transaction.commit(beganTransaction);
    }
    } catch (org.moqui.context.TransactionException e) {
...
<#--spreading code：entity-update-by-condition标签实现-->
<#macro "entity-update-by-condition">
    mapSize = ${.node["set-field"]?size!0}
    if ( mapSize > 0 ){ Map < String, Object > temp_internal_Map =new HashMap < String, Object > ( (int) Math.ceil ( mapSize / 0.75 ) );
    <#list .node["set-field"] as fs><#if fs["@set-if-empty"]?has_content && fs["@set-if-empty"] == "false">_temp_internal_field =basicConvert(<#if fs["@from"]?has_content>${fs["@from"]}<#else>"""${fs.@value}"""</#if><#if fs["@default-value"]?has_content> ?: "${fs["@default-value"]}"</#if><#if fs["@type"]?has_content>, "${fs["@type"]}"</#if>);if (_temp_internal_field){ temp_internal_Map.put('${fs["@field-name"]}',_temp_internal_field);}<#else>temp_internal_Map.put('${fs["@field-name"]}',<#if fs["@type"]?has_content>basicConvert</#if>(<#if fs["@from"]?has_content>${fs["@from"]}<#else>"""${fs["@value"]}"""</#if><#if fs["@default-value"]?has_content>?: "${fs["@default-value"]}"</#if><#if fs["@type"]?has_content>, "${fs["@type"]}"</#if>));</#if></#list>
    org.moqui.entity.EntityFind ef = ec.entity.find("${.node["@entity-name"]}")<#list .node["date-filter"] as df>.condition(<#visit df/>)</#list><#list .node["econdition"] as ec>.condition(<#visit ec/>)</#list><#list .node["econditions"] as ecs>.condition(<#visit ecs/>)</#list><#list .node["econdition-object"] as eco>.condition(<#visit eco/>)</#list>
    ef.updateAll(temp_internal_Map);
    }
</#macro>
