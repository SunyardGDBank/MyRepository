...
class ScreenForm {
    ...
    void addAutoEntityField(EntityDefinition ed, String fieldName, String fieldType, MNode newFieldNode, MNode subFieldNode, MNode baseFormNode) {
        ...
        /**
         *spreading code：获取到idName
         */
        String idName = ed.getFieldInfo(fieldName).name
        ...
        case "edit":
            ...
            */
            if (efType.startsWith("date") || efType.startsWith("time")) {
                ...
                /**
                 * spreading code：
                 * 对rpFlg，clearDCFlag，dbatchDCFlg，cbatchDCFlg，sgnStatusId等字段的下拉选项支持
                 */
            } else if ( "rpFlg"==idName || "clearDCFlag"==idName || "dbatchDCFlg"==idName || "cbatchDCFlg"==idName ) {
                MNode dropDownNode = subFieldNode.append("drop-down", ["allow-empty":"true"])
                dropDownNode.append("option", ["key":"1"])
                dropDownNode.append("option", ["key":"2"])
            } else if ( "sgnStatusId"==idName ) {
                MNode dropDownNode = subFieldNode.append("drop-down", ["allow-empty":"true"])
                dropDownNode.append("option", ["key":"00"])
                dropDownNode.append("option", ["key":"01"])
            }else {
                if (oneRelNode != null) {
