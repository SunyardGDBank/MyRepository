...
public abstract class EntityValueBase implements EntityValue {
    ...
    /**
     * spreading code：
     *标签entity-sequenced-id-primary-dynamic的实现
     * @return
     */
    public EntityValue setDynamicSequencedIdPrimary() {
        EntityDefinition ed = getEntityDefinition();
        ...
        String entityPrimaryName = null;
        String rawPrefix = ed.entityInfo.sequencePrimaryPrefix;
        String rawPrimaryName = ed.entityInfo.sequencePrimaryName;
        if (rawPrefix != null && rawPrefix.length() > 0)
            entityPrefix = localEfi.ecfi.resourceFacade.expand(rawPrefix, null, valueMapInternal);
        if (rawPrimaryName != null && rawPrimaryName.length() > 0)
            entityPrimaryName = localEfi.ecfi.resourceFacade.expand(rawPrimaryName, null, valueMapInternal);
        String sequenceValue = localEfi.dynamicSequencedIdPrimaryEd(ed,entityPrimaryName);
    ...
    private void handleAuditLog(boolean isUpdate, Map oldValues, EntityDefinition ed, ExecutionContextImpl ec) {
        ...
        for (int i = 0; i < fieldInfoList.length; i++) {
            ...
            if ((!isLogUpdate && "true".equals(fieldInfo.enableAuditLog)) || (isUpdate && isLogUpdate)) {
                ...
                //2017-12-28 代码新增开始
                //EntityAuditLog实体添加调用URL地址
                if(ec.getWeb()!=null)
                    parms.put("url", ec.getWeb().getRequestUrl());
                //2017-12-28 代码新增结束
