...
<div id="apps-root"><#-- NOTE: webrootVue component attaches here, uses this and below for template -->
    ...
    <div id="top"><nav class="navbar navbar-inverse"><#--  navbar-fixed-top navbar-static-top --><div class="container-fluid">
        ...
        <div id="navbar-buttons" class="collapse navbar-collapse navbar-ex1-collapse">
            ...
            <#--To change the menu  改变菜单栏样式按钮-->
            <a href="javascript:;" onclick="$('#left').toggle();$('#dynamic-menus').toggle(); $('#content').toggleClass('left-open')" data-toggle="tooltip" data-original-title="To change the menu" data-placement="bottom" class="btn btn-default btn-sm navbar-btn navbar-right"><i class="glyphicon glyphicon-retweet"></i></a>
    ...
    <div id="left">
        ...
        <div class="sunyard-nav-box">
            <ul id="dynamic-menus" class="sunyard-nav">
                <li v-for="(navMenuItem, menuIndex) in navMenuList" class="dropdown">
                    <template v-if="menuIndex < (navMenuList.length - 1)">
                        <#--<m-link v-if="navMenuItem.hasTabMenu" :href="navMenuItem.path" class="active">{{navMenuItem.title}} <i class="glyphicon glyphicon-chevron-right"></i></m-link>-->
                        <template v-if="navMenuItem.subscreens && navMenuItem.subscreens.length > 1">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">{{navMenuItem.title}}</a>
                            <ul class="dropdown-menu">
                                <li v-for="subscreen in navMenuItem.subscreens" :class="{active:subscreen.active}">
                                    <m-link :href="subscreen.pathWithParams">
                                        <template v-if="subscreen.image">
                        ...
                        <#--<m-link v-else :href="navMenuItem.path" class="active">{{navMenuItem.title}} <i class="glyphicon glyphicon-chevron-right"></i></m-link>-->
                    </template>
                </li>
            </ul>
        </div>
    ...
    <#--spreading code：使用新版vapps 去掉页面脚的黑边-->
    <#--<#if hideNav! != 'true'>
