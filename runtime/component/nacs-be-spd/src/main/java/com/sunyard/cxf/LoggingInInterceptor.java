...
public class LoggingInInterceptor extends AbstractLoggingInterceptor {
    ...
    private static LoggingInInterceptor instance = null;
    
    public static AbstractLoggingInterceptor getInstance(){
        if (instance == null){
            instance = new LoggingInInterceptor();
        }
        return instance;
    }
    ...
    protected void logging(Logger logger, Message message) throws Fault {
        ...
        if (!Boolean.TRUE.equals(message.get(Message.DECOUPLED_CHANNEL_MESSAGE))) {
            ...
            if (responseCode != null) {
                buffer.getEncoding().append(responseCode);//.getResponseCode().append(responseCode);
            }
        }
        ...
        if (httpMethod != null) {
            buffer.getContentType().append(httpMethod);//.getHttpMethod().append(httpMethod);
        }
    ...
    protected void logReader(Message message, Reader reader, LoggingMessage buffer) {
        try {
            ...
            CacheAndWriteOutputStream writer = new CacheAndWriteOutputStream(os);
